...
from .matter_power_spt_updated_G3_factor import P_13_regF,P_13_regG,P_13_regFG, Y1_reg_NL, Y2_reg_NL,P_13_F,P_13_G,P_13_FG
...
class FASTPT:
    ...
    def __init__(self, k, nu=None, to_do=None, param_mat=None, low_extrap=None, high_extrap=None, n_pad=None,
        ...
        if self.dd_do:
            ...
            p_mat_reg = np.array([[0, 0, 0, 0], [0, 0, 2, 0], [0, 0, 4, 0], [2, -2, 2, 0], \
                              [1, -1, 1, 0], [1, -1, 3, 0], [2, -2, 0, 1]])
            ...
            self.X_spt_reg = scalar_stuff(p_mat_reg, nu, self.N, self.m, self.eta_m, self.l, self.tau_l)
    ...
    def one_loop_dd_bias_b3nl_density(self, P, P_window=None, C_window=None):
        nu = -2
        #print("I am using this newer version of FASTPT")
        # routine for one-loop spt calculations
        ...
        P22_mat = np.multiply(one_loop_coef, np.transpose(mat))
        P22 = np.sum(P22_mat, 1)
        P13 = P_13_F(self.k_old, Ps)
        P_1loop = P22 + P13
        
        #This is for regularized P22 and P13 integrals
        Ps_reg, mat_reg = self.J_k_scalar(P, self.X_spt_reg, nu, P_window=P_window, C_window=C_window)
        self.Ps_reg = Ps_reg

        P22_mat_reg = np.multiply(one_loop_coef, np.transpose(mat_reg))
        P22_reg = np.sum(P22_mat_reg, 1)
        #print('in density')
        #print(P22_reg.shape)
        P13_reg = P_13_regF(self.k_old, Ps_reg)
        #print(P13_reg.shape)
        P_1loop_reg = P22_reg + P13_reg

        sig4 = np.trapz(self.k_old ** 3 * Ps_reg ** 2, x=np.log(self.k_old)) / (2. * pi ** 2)
        Pd1d2 = 2. * (17. / 21 * mat_reg[0, :] + mat_reg[4, :] + 4. / 21 * mat_reg[1, :])
        Pd2d2 = 2. * (mat_reg[0, :])
        Pd1s2 = 2. * (8. / 315 * mat[0, :] + 4. / 15 * mat[4, :] + 254. / 441 * mat[1, :] + 2. / 5 * mat[5,:] + 16. / 245 * mat[2,:])
        Pd2s2 = 2. * (2. / 3 * mat[1, :])
        Ps2s2 = 2. * (4. / 45 * mat[0, :] + 8. / 63 * mat[1, :] + 8. / 35 * mat[2, :])
        sig3nl = Y1_reg_NL(self.k_old, Ps_reg)

        if (self.extrap):
            _, Ps = self.EK.PK_original(Ps)
            _, P_1loop = self.EK.PK_original(P_1loop)
            _, P_1loop_reg = self.EK.PK_original(P_1loop_reg)
            _, Pd1d2 = self.EK.PK_original(Pd1d2)
            _, Pd2d2 = self.EK.PK_original(Pd2d2)
            _, Pd1s2 = self.EK.PK_original(Pd1s2)
            _, Pd2s2 = self.EK.PK_original(Pd2s2)
        ...
        return P_1loop_reg, Ps, Pd1d2, Pd2d2, Pd1s2, Pd2s2, Ps2s2, sig4, sig3nl
    
    #Function based on the one above to calculate velocity terms
    def one_loop_dd_bias_b3nl_velocity(self, P, P_window=None, C_window=None):
        nu = -2
        #print("I'm using this one")

        # routine for one-loop spt calculations

        # coefficents for one_loop calculation
        #now those calculated from the expansion of (G3)**2
        one_loop_coef = np.array(
            [2 * 851 / 1470., 2 * 871 / 1029., 2 * 128 / 1715., 2 * 1 / 3., 2 * 54 / 35., 2 * 16 / 35., 1 / 3.])

        # get the roundtrip Fourier power spectrum, i.e. P=IFFT[FFT[P]]
        # get the matrix for each J_k component
        #This is for non-regularized P22 and P13
        Ps, mat = self.J_k_scalar(P, self.X_spt, nu, P_window=P_window, C_window=C_window)

        P22_mat = np.multiply(one_loop_coef, np.transpose(mat))
        P22 = np.sum(P22_mat, 1)
        
        #Here calls the P_13_regG function. Will be used to calculate the regularized P_13 integral for the G3 kernel
        #This function contains all of the equations for Z(r) specific to this kernel
        P13G = P_13_G(self.k_old, Ps)
        P_1loop = P22 + P13G
        #This is for regularized P22 and P13 integrals
        Ps_reg, mat_reg = self.J_k_scalar(P, self.X_spt_reg, nu, P_window=P_window, C_window=C_window)
        #print(self.Ps_reg/Ps_reg)

        P22_mat_reg = np.multiply(one_loop_coef, np.transpose(mat_reg))
        P22_reg = np.sum(P22_mat_reg, 1)
        #print('in velocity')
        #print(P22_reg.shape)
        P13_reg = P_13_regG(self.k_old, Ps_reg)
        #print(P13_reg.shape)
        P_1loop_reg = P22_reg + 6.*P13_reg  
        #print(P_1loop_reg.shape)

        #I have not changed this here. I've only been looking at the code calculating the 1-loop PS with linear bias
        
        sig4 = np.trapz(self.k_old ** 3 * Ps_reg ** 2, x=np.log(self.k_old)) / (2. * pi ** 2)
        Pd1d2 = 2. * (13. / 21 * mat_reg[0, :] + mat_reg[4, :] + 8. / 21 * mat_reg[1, :])
        #Pd2d2 = 2. * (mat_reg[0, :])
        ...
        #			return P_1loop, Pd1d2, Pd2d2, Pd1s2, Pd2s2, Ps2s2, sig4, Ps #original
        #return P_1loop, Ps, Pd1d2, Pd2d2, Pd1s2, Pd2s2, Ps2s2, sig4, sig3nl  # new,for consistency
        return P_1loop_reg, P22_reg, P13_reg, Pd1d2, Pd1s2, sig3nl, P_1loop
    
    #Function based on the one above to calculate cross term
    def one_loop_dd_bias_b3nl_cross(self, P, P_window=None, C_window=None):
        nu = -2

        # routine for one-loop spt calculations

        # coefficents for one_loop calculation
        #These are now those calculated from the expansion of (F3*G3)
        one_loop_coef = np.array(
        ...
        # get the roundtrip Fourier power spectrum, i.e. P=IFFT[FFT[P]]
        # get the matrix for each J_k component
        #This is for non-regularized P22 and P13 integrals
        Ps, mat = self.J_k_scalar(P, self.X_spt, nu, P_window=P_window, C_window=C_window)

        P22_mat = np.multiply(one_loop_coef, np.transpose(mat))
        P22 = np.sum(P22_mat, 1)
        
        #Here calls the P_13_regFG function. Will be used to calculate the regularized P_13 integral for the F3+G3 kernel
        #This function contains all of the equations for Z(r) specific to this kernel
        #P13FG = P_13_regFG(self.k_old, Ps)
        P13 = P_13_F(self.k_old, Ps)
        P13G = P_13_G(self.k_old, Ps)
        P13FG=.5*(P13+P13G)
        P_1loop = P22 + P13FG
        ...
        P22_mat_reg = np.multiply(one_loop_coef, np.transpose(mat_reg))
        P22_reg = np.sum(P22_mat_reg, 1)
        P13_reg = 3.*(P_13_regF(self.k_old, Ps_reg)+P_13_regG(self.k_old, Ps_reg))
        #P13_reg=0.5*P_13_regFG(self.k_old, Ps_reg)
        P_1loop_reg = P22_reg + P13_reg  

        #I have not changed this here
        #sig4 = np.trapz(self.k_old ** 3 * Ps_reg ** 2, x=np.log(self.k_old)) / (2. * pi ** 2)
        #Pd1d2 = 2. * (17. / 21 * mat_reg[0, :] + mat_reg[4, :] + 4. / 21 * mat_reg[1, :])
        #Pd2d2 = 2. * (mat_reg[0, :])
        #Pd1s2 = 2. * (8. / 315 * mat[0, :] + 4. / 15 * mat[4, :] + 254. / 441 * mat[1, :] + 2. / 5 * mat[5,
        ...
        #			return P_1loop, Pd1d2, Pd2d2, Pd1s2, Pd2s2, Ps2s2, sig4, Ps #original
        #return P_1loop, Ps, Pd1d2, Pd2d2, Pd1s2, Pd2s2, Ps2s2, sig4, sig3nl  # new,for consistency
        return P_1loop_reg, P22_reg, P13_reg,P_1loop
